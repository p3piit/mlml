...
test_that("fit_gmerf_opt is reproducible with seed", {
  ...
  a <- fit_gmerf_opt(df = df0, ntrees = 40L, max_iter_inn = 3L, max_iter_out = 2L, seed = 42)
  b <- fit_gmerf_opt(df = df0, ntrees = 40L, max_iter_inn = 3L, max_iter_out = 2L, seed = 42)
...
test_that("fit_gmerf_opt gives similar estimates to original fit_gmerf (small iterations)", {
  ...
  res_orig <- fit_gmerf(df = df_cmp, ntrees = 50L, max_iter_inn = 3L, max_iter_out = 2L, seed = 101)
  res_opt  <- fit_gmerf_opt(df = df_cmp, ntrees = 50L, max_iter_inn = 3L, max_iter_out = 2L, seed = 101)
...
test_that("fit_gmerf_opt is consistent with fit_gmerf_small (same optimization family)", {
  ...
  res_small <- fit_gmerf_small(df = df_cmp2, ntrees = 40L, max_iter_inn = 3L, max_iter_out = 2L, seed = 202)
  res_opt   <- fit_gmerf_opt(df = df_cmp2, ntrees = 40L, max_iter_inn = 3L, max_iter_out = 2L, seed = 202)
...
test_that("fit_gmerf_opt shows a wall-clock advantage against the original implementation", {
  ...
  t_orig <- system.time(res1 <- fit_gmerf(df = df_time, ntrees = 60L, max_iter_inn = 3L, max_iter_out = 2L, seed = 303))["elapsed"]
  t_opt  <- system.time(res2 <- fit_gmerf_opt(df = df_time, ntrees = 60L, max_iter_inn = 3L, max_iter_out = 2L, seed = 303))["elapsed"]
...
test_that("fit_gmerf_opt runs on higher dimensional data and returns expected columns", {
  ...
  res_hd <- fit_gmerf_opt(df = df_hd, ntrees = 50L, max_iter_inn = 3L, max_iter_out = 2L, seed = 404)
