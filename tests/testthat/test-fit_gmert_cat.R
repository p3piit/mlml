test_that("fit_gmert_cat returns the expected categorical object structure", {
  df <- gen_sim_cat(
    n = 180,
    n_groups = 12,
    n_vars = 6,
  ...
  expect_true(all(c(
    "trees", "b", "D", "sigma2", "mu", "K", "classes",
    "converged_in", "converged_out", "n_iter",
    "train_ids", "tol", "d_eta"
  ) %in% names(fit)))

  expect_true(is.list(fit$trees))
  expect_equal(length(fit$trees), fit$K - 1)
  expect_true(all(vapply(fit$trees, inherits, logical(1), what = "rpart")))
...
test_that("fit_gmert_cat returns correct stacked dimensions", {
  df <- gen_sim_cat(
    n = 150,
    n_groups = 10,
    n_vars = 5,
  ...
  expect_equal(length(fit$trees), K1)
})

test_that("fit_gmert_cat runs end-to-end and predicts classes without error", {
  df <- gen_sim_cat(
    n = 200,
    n_groups = 10,
    n_vars = 6,
...
test_that("fit_gmert_cat is robust to different seeds/data (smoke test)", {
  df <- gen_sim_cat(
    n = 120,
    n_groups = 6,
    n_vars = 5,
...
test_that("fit_gmert_cat fits intercept-only random-effects model", {
  df <- gen_sim_cat(
    n = 120,
    n_groups = 6,
    n_vars = 5,
...
test_that("fit_gmert_cat rejects binary response", {
  df <- gen_sim(
    n = 100,
    n_groups = 5,
    n_vars = 4,
...
test_that("predict_gmert works with unseen clusters in new data", {
