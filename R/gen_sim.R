...
gen_sim <- function(
  ...
  # interpret n_random == NULL as "random-intercept only"
  ...
  if (!is.null(random_vars)) {
    ...
    if (!has_random) stop("random_vars supplied but n_random is NULL/0")
  ...
  if (n_random > 0 && is.null(random_vars)) {
  ...
  colnames(re_draws) <- c("(Intercept)", if (n_random > 0) paste0("bX", random_vars) else character(0))
  ...
  if (n_random == 0) {
    # only intercept varying by group
    for (i in seq_len(n)) {
      g <- groups[i]
      intercept_part[i] <- re_draws[g, 1]
    }
    rand_contrib <- numeric(n)  # zeros
  } else {
    for (i in seq_len(n)) {
      g <- groups[i]
      intercept_part[i] <- re_draws[g, 1]
