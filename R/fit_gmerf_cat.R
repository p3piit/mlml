...
#'
#' Extends the binary GMERF to K-class categorical responses using the multinomial
#' logit link. The fixed part is represented by K-1 random forests (one per
...
#' inner loop described for GMERF, while replacing direct \eqn{V_i^{-1}} computations
...
fit_gmerf_cat <- function(df,
  ...
  repeat {
    ...
    repeat {
      ...
      # (1.ii) M-step: fit one random forest per contrast
      forests <- vector("list", K1)
      for (k in seq_len(K1)) {
        df_k <- data.frame(y_star = y_star[, k], Xdf)

        forests[[k]] <- ranger::ranger(
          formula = y_star ~ .,
          data = df_k,
          num.trees = ntrees,
          mtry = mtry,
      ...
      # Extract fitted values from forests: N x K1
      fhat <- matrix(0, N, K1)
      for (k in seq_len(K1)) {
        fhat[, k] <- forests[[k]]$predictions
      }
